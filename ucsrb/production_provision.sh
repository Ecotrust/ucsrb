...
/usr/bin/python3 -m venv $VIRTUALENV_DIR &&
    ...
    $PIP install --src ./deps -r $PROJECT_DIR/apps/madrona-features/requirements.txt && \
    $PIP install -e $PROJECT_DIR/apps/madrona-manipulators && \
    $PIP install --src ./deps -r $PROJECT_DIR/apps/madrona-manipulators/requirements.txt && \
    $PIP install -e $PROJECT_DIR/apps/mp-accounts && \
    $PIP install --src ./deps -r $PROJECT_DIR/apps/mp-accounts/requirements.txt && \
    $PIP install -e $PROJECT_DIR/apps/mp-data-manager && \
    $PIP install --src ./deps -r $PROJECT_DIR/apps/mp-data-manager/requirements.txt && \
    $PIP install -e $PROJECT_DIR/apps/mp-visualize && \
    $PIP install --src ./deps -r $PROJECT_DIR/apps/mp-visualize/requirements.txt && \
    $PIP install -e $PROJECT_DIR/apps/p97-nursery && \
    $PIP install --src ./deps -r $PROJECT_DIR/apps/p97-nursery/requirements.txt && \
    $PIP install -e $PROJECT_DIR/apps/madrona-analysistools && \
    $PIP install --src ./deps -r $PROJECT_DIR/apps/madrona-analysistools/requirements.txt && \
